...
public class HelloApplication extends Application {
    ...
    static {
        try (InputStream resourceAsStream = HelloApplication.class.getResourceAsStream("/logging.properties")) {
            if (resourceAsStream != null) {
                LogManager.getLogManager().readConfiguration(resourceAsStream);
            }
        } catch (IOException ex) {
            Logger.getLogger(HelloApplication.class.getName()).log(Level.SEVERE, "Error opening logging.properties file", ex);
        }
    }
    ...
    public void start(Stage stage) {
        ...
        try {
            matches = langTool.check("Привіт. Це помілка.");
        } catch (IOException e) {
            e.printStackTrace();
        }
...
//        menuBar.setUseSystemMenuBar(true);
        ...
        scene.getStylesheets().add(HelloApplication.class.getResource("css/richtexteditordemo.css").toExternalForm());
